...
#
# Inputs: Files: BTCTSV.tsv - labelled training data, BTC_posts_1.txt - unseen reddit post titles from bitcoin subreddit
# Output: Sentiment scores for each day seen in BTC_posts_1.txt
# =========================================================================
import numpy as np
import pandas as pd
...
df = pd.read_csv('BTCTSV.tsv', delimiter='\t', header=None)
...
# test sentiment outputs for 3 test sentences and print output arrays
new_input_ids = torch.tensor(tokenizer.encode("Bitcoin is amazing", add_special_tokens=True)).unsqueeze(0)
...
new_input_ids = torch.tensor(tokenizer.encode("Buy more!", add_special_tokens=True)).unsqueeze(0)
new_outputs = model(new_input_ids)
new_last_hidden_states = [new_outputs[0].detach().numpy()[0][0]]
score2 = lr_clf.predict_proba(new_last_hidden_states)
print("Score 2", score2)

new_input_ids = torch.tensor(tokenizer.encode("Bitcoin is very bad", add_special_tokens=True)).unsqueeze(0)
new_outputs = model(new_input_ids)
new_last_hidden_states = [new_outputs[0].detach().numpy()[0][0]]
...
# open testings posts
posttxtBTC = open('BTC_posts_1.txt', 'r')

# create lists to hold posts by day/all days of posts
all_days_BTC = []
day_posts_BTC = []
for line in posttxtBTC:
    line = line.strip('\n')
    # check for new date
    if line.__contains__('END DAY'):
        # add day to main list and reinitialize
        all_days_BTC.append(day_posts_BTC)
        day_posts_BTC = []
    else:
        # add post to day
        day_posts_BTC.append(line)
print("Length", len(all_days_BTC))

print("Scores:")
# create list of scores
all_scores = []
for day in all_days_BTC:
